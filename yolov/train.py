...
def organize_labels_and_frames(
    image_dir, label_dir, output_image_dir, output_label_dir
):
    ...
    def process_video(video_folder, output_image_dir, output_label_dir, frame_index):
        ...
        for img_path in train_images:
            ...
            label_path = os.path.join(
                video_label_dir, os.path.basename(img_path).replace(".jpg", ".txt")
            )
    ...
    for video_folder in video_folders:
        global_frame_index = process_video(
            video_folder, output_image_dir, output_label_dir, global_frame_index
        )
    ...
    def display_file_counts():
        ...
        for split_folder in ["train", "val"]:
            image_count = len(
                glob.glob(os.path.join(output_image_dir, split_folder, "*.jpg"))
            )
            label_count = len(
                glob.glob(os.path.join(output_label_dir, split_folder, "*.txt"))
            )
            print(
                f"{split_folder.capitalize()} set - Images: {image_count}, Labels: {label_count}"
            )
...
organize_labels_and_frames(
    output_image_directory, label_root, output_image_directory, label_root
)

print("Finished processing frames and labels.")
base_dir = os.path.abspath(
    "dataset"
)  # Adjust this as needed, e.g., to the full path of your dataset directory
...
def train_model():
    model = (
        load_model()
    )  # Load the model (either best.pt, a fallback pt file, or the default YOLOv8 model)
