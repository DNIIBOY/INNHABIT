...
def load_model():
    ...
    if pt_files:
        ...
        model = YOLO(os.path.join(".", pt_files[0]))
        return model
    ...
    return YOLO("yolo11n.pt")  # Default YOLOv8 model, adjust if necessary
...
def train_model():
    ...
    model.train(
        ...
        epochs=15,  # Train longer for better recall  
        imgsz=640,  # Higher resolution for small objects  
        batch=32,  # Balance VRAM usage and training stability  
        patience=10,  # Stop early if no improvement  
        lr0=0.0001,  # Lower learning rate for stability  
        lrf=0.00005,  # Very low final LR for fine-tuning  
        cos_lr=True,  # Smooth learning rate decay  
        optimizer="SGD",  # Stable and good for recall  
        momentum=0.94,  # Helps recall  
        weight_decay=0.0005,  # Regularization  
        hsv_h=0.02, hsv_s=0.8, hsv_v=0.5,  # Keep augmentations strong  
        flipud=0.5, fliplr=0.5,  
        degrees=10,  # Reduce rotation (extreme rotations can hurt recall)  
        translate=0.2, scale=0.7, shear=0.2,  
        mosaic=1.0, mixup=0.5,  # Stronger mixup for better generalization  
        iou=0.2,  # Lower IoU threshold so more objects are detected  
        conf=0.1,  # Lower confidence threshold (detects more objects)  
    )
