            ...
            for file_name in file_list:
                ...
                if file_name.endswith("/"):  # This indicates a directory
                    ...
                    new_path = os.path.join(
                        extract_to_dir, os.path.relpath(file_name, top_level_dir)
                    )
                    os.makedirs(
...
def augment_and_create_video_from_zip_mp4(video_path, zip_path, output_name):
    ...
    extracted_folder = zip_path.rstrip(
        ".zip"
    )  # Remove the .zip extension for folder name
    ...
    out_video = cv2.VideoWriter(
        augmented_video_path, fourcc, 30.0, (frame.shape[1], frame.shape[0])
    ...
    while success:
        ...
        if os.path.exists(label_file):
            ...
            for line in lines:
                ...
                new_bbox = adjust_bounding_box_for_augmentation(
                    (x_center, y_center, width, height), frame.shape, transformation
                )
                new_bboxes.append(
                    f"{class_id} {new_bbox[0]} {new_bbox[1]} {new_bbox[2]} {new_bbox[3]}"
                )
            ...
            if new_bboxes:
                ...
                print(
                    f"No bounding boxes after augmentation for frame_{frame_index:06d}"
                )
    ...
    folder_name_in_zip = (
        f"{output_name}"  # Name of the folder you want inside the zip file
    )
    ...
    if label_files:
        with zipfile.ZipFile(zip_file_name, "w") as zipf:
            for label_file in label_files:
                ...
                zipf.write(
                    label_file,
                    os.path.join(folder_name_in_zip, os.path.basename(label_file)),
                )
